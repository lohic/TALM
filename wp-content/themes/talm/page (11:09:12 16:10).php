	...
	<div id="content" class="right">
		<?php
			...
			if (count($ancetre)>0 || $post->ID==13){
						...
						if(get_sub_field("page_liee_texte_image")){
							foreach(get_sub_field("page_liee_texte_image") as $p){	
							    ...
							    echo '</aside>
							                ...
							                <aside class="en_plus">';
							    foreach($value['liens_supplémentaires'] as $lien){
							    	echo '<a href="'.$lien['url_lien'].'" class="lien_normal">'.$lien['titre_lien'].'</a>';
							    }
				...
				$values = get_field('structure_page');
				if($values){
	                        ...
	                        case 'bloc_onglet' :
	                            ...
	                            echo '<div class="full">
							            <article class="texte_avec_onglet">
							                <header>
							                    <h2>'.$value['titre_bloc_onglet'].'</h2>
							                </header>
							                <aside>
							                    <ul>';
							    foreach($value['onglet'] as $onglet){
							    	$compteur++;
							    	echo '<li class="lien_onglet" id="onglet_'.$compteur.'">
							                            <a href="#" class="">'.$onglet['titre_onglet'].'</a>
							                            <div class="content" id="content_onglet_'.$compteur.'">
							                                <h3>'.$onglet['titre_onglet'].'</h3>
							                                '.$onglet['texte_onglet'].'
							                            </div>
							                        </li>';
							    }
